...
```{r}
era5shceof <- readRDS("~/Downloads/era5.Rds") 
```

```{r}
rbind(shceof = era5shceof, 
      tesis = era5, idcol = "type") %>% 
  .[lon == 60 & lat == -20] %>% 
  .[lev == 200] %>% 
  ggplot(aes(time, hgt)) +
...
```{r}
with_o3_shceof <- readRDS("~/Downloads/with_o3.Rds")
```
...
La correlación temporal entre la amplitud de la onda 1 de CTO y la amplitud del cEOF1 es `r wave1_cor[with == "amplitude" & season == "SON"]$text`, mientras que la correlación entre sus fases es `r wave1_cor[with == "phase" & season == "SON"]$text`.
