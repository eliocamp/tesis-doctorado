...
Sin embargo, al analizar los índices típicamente utilizados en la literatura para caracterizar la onda 3, como el propuesto por Raphael (\protect\hyperlink{ref-raphael2004}{2004}) o el computado por análisis de Fourier, se observó que éstos no son capaces de caracterizar propiedades importantes de la misma, como su propagación meridional, su amplitud variable a lo largo de cada círculo de latitud y la variación de su fase fase a lo largo del año.
...
Las tendencias positivas del SAM de verano y otoño en la troposfera documentadas por estudios previos (p.e. . \protect\hyperlink{ref-fogt2020}{Fogt y Marshall 2020} y sus referencias) son únicamente explicadas por el S-SAM.
...
Hay evidencia de que el SAM está evolucionando hacia ser más asimétrico en verano, contrario a lo observado por Fogt, Jones y Renwick (\protect\hyperlink{ref-fogt2012}{2012}), aunque la discrepancia puede deberse a las diferencias metodológicas o al período analizado.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{n\_cmip }\OtherTok{\textless{}{-}} \FunctionTok{subset}\NormalTok{(}\FunctionTok{readRDS}\NormalTok{(eliotesis}\SpecialCharTok{::}\FunctionTok{data\_path}\NormalTok{(}\StringTok{"derived"}\NormalTok{, }\StringTok{"sims.Rds"}\NormalTok{)), experiment\_id }\SpecialCharTok{==} \StringTok{"historical"}\NormalTok{)}\SpecialCharTok{$}\NormalTok{source\_id}
\NormalTok{n\_cmip }\OtherTok{\textless{}{-}} \FunctionTok{length}\NormalTok{(}\FunctionTok{unique}\NormalTok{(n\_cmip))}
\end{Highlighting}
\end{Shaded}
