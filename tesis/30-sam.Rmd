...
Lo opuesto se encuentra para la fase negativa [@jones2019].
...
La señal relacionada con el SAM en las anomalías de precipitación también es positiva en latitudes altas y negativos en latitudes medias, aunque con aún mayores desviaciones respecto de la simetría zonal [@lim2016].
En particular, la relación entre el SAM y la precipitación en el Sudeste de Sudamérica (SESA) en escalas interanuales depende fuertemente de las anomalías de circulación zonalmente asimétrica asociadas al SAM y presenta importantes variaciones decadales [@silvestri2009; @rosso2018].
...
Como discutimos en las secciones anteriores el ENSO o la variabilidad tropical en general afecta a los extratrópicos del HS a través de trenes de ondas de Rossby [@mo1987; @kidson1988; @karoly1989] que pueden proyectarse fuertemente sobre las anomalías zonales asociadas al SAM en el sector del Pacífico [ej. @silvestri2009; @vera2018].
...
En escalas más largas, investigaciones previas han documentado a lo largo del siglo 20 y lo que va del 21, tendencias positivas en el SAM utilizando diferentes índices, sobre todo en verano y otoño austral [ej., @fogt2020 y sus referencias].
...
## Datos y métodos Métodos

### Datos

Se utilizan las mismas fuentes de datos que en capítulos anteriores.
En este capítulo usamos en particular datos de altura geopotencial, temperatura a 2 metros del conjunto de datos ERA5 y precipitación del conjunto de datos CMAP para el período `r periodo`.
...
### D**escomposición de las componentes del SAM** {#definition-of-indices}
...
Siguiendo a @baldwin2001, ampliamos esa definición verticalmente y utilizamos el término SAM para referirnos al primer EOF de las anomalías mensuales de altura geopotencial al sur de 20º S en cada nivel vertical considerado y que de ahora en adelante si bien llamaremos SAM, se refiere al SAM "completo" que incluye tanto la componente simétrica como la asimétrica.
...
En la estratosfera, las correlaciones instantáneas caen a un mínimo de `r signif(cors[lag == 0 & lev < 100][which.min(acf)]$acf, 2)` en `r cors[lag == 0 & lev < 100][which.min(acf)]$lev` hPa y luego aumentan nuevamente monotónicamente con la altura hasta el nivel más alto considerado  (aunque los resultados cerca del tope superior representado en los modelos deben interpretarse con cuidado).
...
Este eje se alinea con el defasaje del monopolo encontrado para el SAM (Fig. \@ref(fig:2d-regr-cap).a.1).
...
En la troposfera, el patrón espacial descrito por la regresión asociada al SAM muestra la ya conocida combinación de una estructura anular zonalmente simétrica en la zona polar con asimetrías zonales en forma de onda-3 en las latitudes medias (Fig. \@ref(fig:2d-regr)b.1, [@fogt2012]).
...
(ref:enso-cor-table-cap) Correlación entre los índices del SAM y el ONI. En negrita, las correlaciones con p-valor ajustado por FDR menores a 0.01.
...
Esto promueve un flujo anómalo del sudeste sobre el SESA que inhibe el flujo del chorro de baja altura desde Sudamérica hacia la región [ej. @silvestri2009; @zamboni2010].
...
(ref:sam-eof-vertical-cap) Coeficiente de determinación ($r^2$) entre la fase de 0º (fila a) y 90º (fila b) de los cEOFs con el SAM, A-SAM y S-SAM para cada nivel durante el período `r periodo`. Las líneas gruesas representan valores con p-valor menor a 0.01 ajustado por FDR.
