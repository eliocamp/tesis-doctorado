...
La correlación entre ambos índices es muy alta, con valores superiores al 75% de la varianza compartida en toda la tropósfera y un máximo de `r scales::percent(max_r2[cEOF == "cEOF2" & term == "asym" & season == "SON"]$estimate^2, 2)` en `r max_r2[cEOF == "cEOF2" & term == "asym" & season == "SON"]$lev` hPa. 
...
En SON (panel 4), el cEOF2 tiene una fase similar a $\pm$ 90º un `r scales::percent(imaginary[panel == 4]$percent)` de los años, indicando que es la fase más común. 
...
`r todo("Faltaría analizar las otras estaciones, pero quiero ver lo anterior primero")`
