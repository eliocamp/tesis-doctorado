...
La correlación temporal entre las amplitudes de la onda planetaria 1 del COT y la amplitud del cEOF1 es `r wave1_cor[with == "amplitude"]$text`, mientras que la correlación entre sus fases es `r wave1_cor[with == "phase"]$text`.
...
De hecho, existe una correlación significativa y muy alta (`r enso_cor[cEOF == "cEOF2" & part == "Imaginario"]$text`) entre el ONI y la serie temporal 90º cEOF2.
...
De forma coherente, la correlación entre el 90º cEOF2 y el DMI es `r dmi_cor[cEOF == "cEOF2" & part == "Imaginario"]$text`.
...
Como resultado, la correlación entre el 0º cEOF2 y ENSO no es significativa (`r enso_cor[cEOF == "cEOF2" & part == "Real"]$text`).
...
El $r^2$ correspondiente al ajuste es `r signif(summary(cos_model)$r.squared, 2)`, estadísticamente significativo con valor p `r report_p(coef(summary(cos_model))[4])`, lo que indica una relación casi sinusoidal entre estas dos variables.

La correlación entre la magnitud absoluta del ONI y la amplitud del cEOF2 es `r cor_enso_mag$text`.
Sin embargo, esta relación está determinada principalmente por los tres años con los eventos ENSO más intensos del periodo (`r knitr::combine_words(year(gdata[order(-oni)][1:3]$time))`), que coinciden con los tres años con la magnitud CEOF2 más intensa (no se muestra).
Si se eliminan esos años, la correlación deja de ser significativa (`r cor_enso_mag_outlier$text`).
Además, incluso cuando se utilizan todos los años, la correlación de Spearman -que es robusta frente a los valores atípicos- tampoco es significativa (`r signif(cor_enso_spearman$estimate, 2)`, valor p `r report_p(cor_enso_spearman$p.value)`).
