...
(ref:cor-lev-cap) Correlación entre S-SAM y A-SAM en cada nivel instantáneas y con un mes de defasaje para el período `r periodo`.
...
(ref:r-squared-trend-cap) Tendencias lineales (en ) Linear trends (in percent per decade) of the variance explained by A-SAM and S-SAM at each level and for each trimester for the period `r periodo`. Shading indicates the 95 confidence interval.
...
En la troposfera, la única tendencia significativa es la de DJF, en la que el A-SAM tiene una tendencia positiva de alrededor del 2% por década, lo que sugiere que el DJF SAM se ha vuelto más asimétrico en el período de `r min(year(main_period))` a `r max(year(main_period))`
