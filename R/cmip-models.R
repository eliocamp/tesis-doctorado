cmip_models <- function(cmip_root = "/shera/datos/CMIP/") {

  # Tuve que optimizar algunas cosas para poder parsear miles de archivos

  cli::cli_alert_info("Buscando archivos de CMIP6 disponibles...")
  ...
  sims <- sims[, .SD[physics == physics[1] & init == init[1] & forcing == forcing[1]],
    ...
    .[, .SD[data.table::uniqueN(ensemble) >= 5], by = source_id]
  ...
  cli::cli_alert_info("Buscando archivos de DAMIP disponibles...")
