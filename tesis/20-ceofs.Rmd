...
explained <- eof_naive[, eof[[1]]$sdev, by = lev] %>% 
  .[order(lev, EOF)] %>% 
  .[, scales::percent(r2, accuracy = .1, decimal.mark = ",")]
...
Por lo tanto, se eligió el argumento del cEOF1 de forma que la serie temporal correspondiente al cEOF1 de 0º tenga la máxima correlación con la onda zonal 1 del CTO entre `r combine_words(LatLabel(o3wave_lats))`.
...
El primer modo (cEOF1) explica el `r scales::percent(ceof$sdev[cEOF == "cEOF1"]$r2, 0.1, decimal.mark = ",")` de la varianza de las anomalías zonales, mientras que el segundo modo (cEOF2) explica una fracción menor (`r scales::percent(ceof$sdev[cEOF == "cEOF2"]$r2, 0.1, decimal.mark = ",")`).
...
(ref:ceofs-1-cap) Patrones espaciales de los dos primeros cEOF de las anomalías zonales de altura geopotencial de SON en 50 (columna a) y 200 (columna b) hPa para el período `r periodo`. El sombreado corresponde a la fase 0º y los contornos, a la fase 90º. La proporción de varianza explicada por cada modo con respecto a la media zonal está indicada entre paréntesis. Unidades arbitrarias.
...
(ref:t-vertical-cap) Regresión de anomalías zonales de temperatura (sombrado, K) y razón de mezcla de ozono (contornos, valores negativos en línea punteada, etiquetas en partes por mil millón en masa) promediados entre `r combine_words(LatLabel(o3wave_lats))` en SON con la fase de 0º (a) y de 90º (b) del cEOF1 para el período `r periodo`.
...
plot <- cut(ceof, 2)$left %>% 
  ...
  geom_rug() +
  scale_y_continuous("Número de años",
                     sec.axis = sec_axis(~.x/N,
                                         name = "Proporción de años",
                                         labels = scales::percent)) +
  scale_x_continuous("Fase", trans = scales::reverse_trans(),
...
oni_dmi_parciales <- gdata %>%
  .[cEOF == "cEOF2"] %>%
  .[, semipartial_cor(hgt, dmi, oni), by = .(cEOF, part)] %>% 
  .[, text := paste0(scales::number(semipartial_correlation, accuracy = 0.01, decimal.mark = ","), " (p-valor ", 
                     report_p(p.value), ")")]
```
...
plot_sst_psi <- function(sst_regr, psi_regr, min.mag = 0.002) {
  ...
  variables <- c("TSM", "Función corriente")
  ...
  stream_gdata %>% 
    ...
    scale_fill_divergent_discretised(variables[1],
                                     labels = label_one_less,
    ...
    geom_qmap() +
    scale_fill_divergent_discretised(variables[2],
                                     labels = label_one_less,
                                     low = scales::muted("red"),
                                     high = scales::muted("blue"),
                                     guide = guide_colorsteps_bottom(15, show.limits = FALSE,
                                                                     order = 99)) +
    grid_panel(grob_spoke, aes(angle = -angle, 
                               var = factor(variables[2], levels = variables)), 
               inherit.aes = FALSE) +
    scale_x_continuous(NULL, expand = c(0, 0), 
                       breaks = seq(0, 360, by = 60),
...
Sin embargo, la correlación semiparcial es de `r oni_dmi_parciales[cEOF == "cEOF2" & part == "Imaginario" & term == "dmi"]$text`, indicando que el DMI explica poca varianza de la fase de 90º del cEOF2 por sí mismo.
...
El DMI explica, independientemente, sólo un `r scales::percent(oni_dmi_parciales[cEOF == "cEOF2" & part == "Imaginario" & term == "dmi"]$semipartial_correlation^2, accuracy = 0.1)` de la varianza, mientras que el ONI explica un `r scales::percent(oni_dmi_parciales[cEOF == "cEOF2" & part == "Imaginario" & term == "oni"]$semipartial_correlation^2, accuracy = 0.1)` por sí mismo.
...
equation <- bquote(ONI  * {phantom() == phantom()} * .(signif(cos_model[, coef(model[[1]])[1]], 2)) * sin(fase) )
...
plot_pp <- function(pp_regr, t2m_regr, hgt850_regr) {
  ...
  pp_regr %>%
    ...
    grid_panel(grob_spoke, aes(angle = -angle, var = factor(variables[1], levels = variables)), 
               inherit.aes = FALSE) +
...
(ref:pp-temp-2-cap) Regresión de la precipitación (correlación, columna 1) y la temperatura de 2 metros (K, sombreado) y la altura geopotencial de 850 hPa (m, contornos, columna 2) sobre diferentes fases de cEOF2 para el trimestre SON del periodo `r periodo`. `r pval_contours()`.
